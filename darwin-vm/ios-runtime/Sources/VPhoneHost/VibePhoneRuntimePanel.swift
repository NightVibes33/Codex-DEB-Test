...
@MainActor
final class VibePhoneRuntimeModel: ObservableObject {
    ...
    func stop() {
        ...
        detailText = "The interpreter will return to the VibePhone host."
        updateCounters()
    }
    ...
    private var firmwareDirectory: URL {
        ...
        return base.appendingPathComponent("VibePhone/DefaultPhone/Firmware", isDirectory: true)
    }
...
struct VibePhoneRuntimePanel: View {
    @StateObject private var model = VibePhoneRuntimeModel()
        ...
        NavigationStack {
            List {
                Section {
                    ...
                    Text("Apple firmware is not bundled. Imported artifacts remain inside VibePhone's app container.")
                }

                Section("Apple boot artifacts") {
                    ForEach(VibePhoneRuntimeModel.Artifact.allCases) { artifact in
                        HStack(spacing: 12) {
            ...
            .navigationTitle("VibePhone Runtime")
            .toolbar {
...
struct VibePhoneRuntimeOverlay: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ZStack(alignment: .topTrailing) {
            ...
            .accessibilityLabel("Open VibePhone Runtime")
            .padding(.top, 12)
            .padding(.trailing, 12)
        }
        .sheet(isPresented: $showingRuntime) {
            VibePhoneRuntimePanel()
        }
    }
}
