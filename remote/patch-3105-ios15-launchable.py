...
BUNDLE_ID = 'com.nightvibes.app.threeoneosfive1'
OLD_BUNDLE_ID = 'com.apple.mobile.MobileHouseArrest'

if len(sys.argv) != 3:
    raise SystemExit('usage: patch-3105-ios15-launchable.py APP_DIR TEXTRO_BIN')
...
# Install the UIKit/iOS 15 fallback entrypoint and lower the executable target.
code_off = 0x2c0100
if any(b[code_off:code_off+len(blob)]):
...
for _ in range(ncmds):
    ...
    p += cmdsize
if main_found != 1 or build_found < 1 or not fixups:
    raise SystemExit('required load commands missing')

dataoff, datasize = fixups
_, _, imports_offset, _, imports_count, imports_format, _ = struct.unpack_from('<7I', b, dataoff)
if imports_format != 1:
    raise SystemExit('unexpected chained import format')
for i in range(imports_count):
    off = dataoff + imports_offset + 4*i
    v = struct.unpack_from('<I', b, off)[0]
    struct.pack_into('<I', b, off, v | (1 << 8))
open(binary, 'wb').write(b)
os.chmod(binary, 0o755)

# Give the Dopamine compatibility build a normal LaunchServices identity.
...
info['ThreeOneOSFiveOriginalBundleIdentifier'] = OLD_BUNDLE_ID
with open(plist_path, 'wb') as f:
    plistlib.dump(info, f, fmt=plistlib.FMT_BINARY, sort_keys=False)

# Add an ad-hoc signature carrying the jailbreak privileges TrollStore will preserve/re-sign.
b = bytearray(open(binary, 'rb').read())
...
for _ in range(ncmds):
    ...
    if cmd == 0x19:
        ...
        if seg == b'__LINKEDIT':
            linkedit = p
    if cmd == 0x1d:
        raise SystemExit('unexpected existing signature')
    p += cmdsize
if linkedit is None:
    raise SystemExit('LINKEDIT missing')
lc_off = p
if lc_off + 16 >= 0x4000:
    raise SystemExit('no load command padding')
sigoff = (len(b) + 15) & ~15
if sigoff > len(b):
    b.extend(b'\0' * (sigoff - len(b)))
...
ent = {
    ...
    'com.apple.private.security.container-required': BUNDLE_ID,
...
cd_off = 28
ent_off = (cd_off + cd_len + 3) & ~3
sig_len = ent_off + len(ent_blob)
struct.pack_into('<IIII', b, lc_off, 0x1d, 16, sigoff, sig_len)
struct.pack_into('<I', b, 16, ncmds + 1)
struct.pack_into('<I', b, 20, sizeofcmds + 16)
fileoff = struct.unpack_from('<Q', b, linkedit + 40)[0]
new_filesize = (sigoff + sig_len) - fileoff
new_vmsize = (new_filesize + 0x3fff) & ~0x3fff
struct.pack_into('<Q', b, linkedit + 48, new_filesize)
struct.pack_into('<Q', b, linkedit + 32, new_vmsize)
...
cd[38] = 0
cd[39] = 12
struct.pack_into('>I', cd, 40, 0)
struct.pack_into('>I', cd, 44, 0)
struct.pack_into('>I', cd, 48, team_off)
cd[ident_off:ident_off+len(ident)] = ident
cd[team_off:team_off+len(team)] = team
cd[hashes_base:hashes_base+32] = hashlib.sha256(ent_blob).digest()
for i in range(n_code):
    page = bytes(b[i*4096:min((i+1)*4096, code_limit)])
    st = hash_off + i*32
    cd[st:st+32] = hashlib.sha256(page).digest()

sig = bytearray(sig_len)
struct.pack_into('>III', sig, 0, 0xfade0cc0, sig_len, 2)
struct.pack_into('>II', sig, 12, 0, cd_off)
struct.pack_into('>II', sig, 20, 5, ent_off)
sig[cd_off:cd_off+len(cd)] = cd
sig[ent_off:ent_off+len(ent_blob)] = ent_blob
b.extend(sig)
open(binary, 'wb').write(b)
os.chmod(binary, 0o755)

final_hash = hashlib.sha256(open(binary, 'rb').read()).hexdigest()
print('LAUNCHABLE_PATCHED_BINARY=1')
print('patched_binary_sha256=' + final_hash)
print('plist_minimum_os=' + info['MinimumOSVersion'])
print('plist_bundle_id=' + info['CFBundleIdentifier'])
