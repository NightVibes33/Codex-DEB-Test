...
printf '%s\n' '=== Install and verify Gif2Ani 3.5.6 SnowBoard gallery on iPad ==='
...
test "$(dpkg-deb -f "$TMP" Version)" = '3.5.6'
...
echo 'installed_version_and_legacy_ui=passed'
...
seen={}
for theme in snowboard['themes']:
    seen.setdefault(theme['package'], theme)
selected=sorted(seen.values(), key=lambda x:int(x['bytes']))[:2]
rows=[]
for theme in selected:
    fields=[theme['package'],str(theme['bytes']),theme['sha256'],theme['downloadURL'],theme['effectiveURL'],theme['archiveSubpath'],str(theme['mediaFiles']),theme['name']]
...
TAB="$(printf '\t')"
INDEX=0
while IFS="$TAB" read -r PACKAGE EXPECTED_BYTES EXPECTED_SHA DOWNLOAD_URL EXPECTED_EFFECTIVE ARCHIVE_SUBPATH EXPECTED_MEDIA DISPLAY_NAME; do
  INDEX=$((INDEX + 1))
  ...
  printf 'snowboard_test_%s_effective_url=%s\n' "$INDEX" "$EFFECTIVE_URL"
  ...
  THEME_DIR="$EXTRACTED/$ARCHIVE_SUBPATH"
  test -d "$THEME_DIR"
  ACTUAL_MEDIA="$(find "$THEME_DIR" -type f \( -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.gif' -o -iname '*.webp' \) | wc -l | tr -d ' ')"
  test "$ACTUAL_MEDIA" = "$EXPECTED_MEDIA"
  printf 'snowboard_test_%s=passed|%s|%s|bytes=%s|media=%s|owner_uid=%s\n' "$INDEX" "$PACKAGE" "$DISPLAY_NAME" "$ACTUAL_BYTES" "$ACTUAL_MEDIA" "$OWNER_UID"
done < "$WORK/packages.tsv"
test "$INDEX" = '2'
echo 'two_real_snowboard_mobile_downloads=passed'
...
echo 'gif2ani_356_physical_snowboard_downloads=success'
