...
for p in /var/jb /var/jb/usr/bin /var/jb/bin /var/jb/usr/sbin /var/jb/Library/MobileSubstrate/DynamicLibraries; do [ -e "$p" ] && echo "present=$p"; done
...
for d in /var/jb/Library/MobileSubstrate/DynamicLibraries /Library/MobileSubstrate/DynamicLibraries; do
  if [ -d "$d" ]; then echo "directory=$d"; ls -lat "$d" 2>/dev/null | head -n 160; fi
done
...
for f in /var/mobile/Library/Preferences/com.apple.powerlog.plist /var/mobile/Library/BatteryLife/CurrentPowerlog.PLSQL /var/mobile/Library/BatteryLife/Archives /var/db/Battery; do
  if [ -e "$f" ]; then echo "power_path=$f"; ls -lah "$f" 2>/dev/null | head -n 60; fi
done
if have plutil && [ -f /var/mobile/Library/Preferences/com.apple.powerlog.plist ]; then plutil -p /var/mobile/Library/Preferences/com.apple.powerlog.plist 2>/dev/null | head -n 180 || true; fi
if have sqlite3 && [ -f /var/mobile/Library/BatteryLife/CurrentPowerlog.PLSQL ]; then echo '-- powerlog db tables --'; sqlite3 /var/mobile/Library/BatteryLife/CurrentPowerlog.PLSQL '.tables' 2>/dev/null | head -n 40 || true; fi

section 'CPU / MEMORY SNAPSHOT 1'
if have top; then top -l 1 -n 35 -o cpu 2>/dev/null | head -n 140 || top -l 1 2>/dev/null | head -n 140 || true; fi
ps ax -o pid,ppid,%cpu,%mem,etime,state,comm 2>/dev/null | head -n 180 || ps -A 2>/dev/null | head -n 180 || true
...
section 'LAUNCHD SUMMARY'
if have launchctl; then launchctl list 2>/dev/null | head -n 500 || true; fi

section 'RECENT CRASH / JETSAM / PANIC NAMES'
for root in /var/mobile/Library/Logs/CrashReporter /Library/Logs/CrashReporter; do
  if [ -d "$root" ]; then
    echo "crash_root=$root"
    find "$root" -maxdepth 2 -type f -mmin -720 -print 2>/dev/null | head -n 180
  fi
done

section 'NETWORK / RADIO STATE'
if have ifconfig; then ifconfig 2>/dev/null | head -n 160 || true; fi

section 'CPU / MEMORY SNAPSHOT 2 AFTER 5S'
sleep 5
if have top; then top -l 1 -n 35 -o cpu 2>/dev/null | head -n 140 || top -l 1 2>/dev/null | head -n 140 || true; fi
ps ax -o pid,ppid,%cpu,%mem,etime,state,comm 2>/dev/null | head -n 180 || ps -A 2>/dev/null | head -n 180 || true

section 'FINAL BATTERY SAMPLE'
if have ioreg; then ioreg -r -c AppleSmartBattery -l 2>/dev/null | grep -Ei 'AppleRaw|CycleCount|CurrentCapacity|MaxCapacity|DesignCapacity|NominalChargeCapacity|Voltage|Amperage|InstantAmperage|Temperature|BatteryHealth|FullyCharged|ExternalConnected|IsCharging|TimeRemaining' | head -n 250 || true; fi
